...
<?php
		if (isset($_GET['submit1'])){
                        if (!ctype_alpha($_GET['dato1'])) {
                            echo "No se permiten caracteres no alfanuméricos";
                            exit();
                        }
		...
		if (isset($_GET['submit2'])){
                        if (!is_numeric($_GET['dato2'])) {
                            echo "No se permiten caracteres no numéricos";
                            exit();
                        }
                        $tmp = htmlspecialchars($_GET['dato2']);
			echo "<form method = get> Introducido el dato 2: <input type = text name='nuevodato2' value = '{$tmp}'>" ;
			exit;
		}
		
		if (isset($_GET['submit3'])){
                        if ($_GET['dato3'] != 'Rojo' || $_GET['dato3'] != 'Verde') {
                            echo "Elige rojo o verde";
                            exit();
                        }
